...
import AttendenceModal from "./components/ModelComp";
import check from "../../images/check.png";
import cross from "../../images/bgcross.png";
...
const EmployeeAttendance = () => {
    ...
    const [modal, setModal] = useState(false);
    const [selectedAttendance, setSelectedAttendance] = useState(null); // State to hold selected attendance object

    const showModal = (attendance) => {
        setSelectedAttendance(attendance); // Set selected attendance object
        setModal(true);
    };
    ...
    useEffect(() => {
        const fetchEmployeesAndAttendance = async () => {
            try {
                ...
                const attendanceData = await getAttendanceOfAllEmployees();
                setAttendanceData(attendanceData);

                // Calculate days of the current month
                const today = new Date();
                const numberOfDays = new Date(today.getFullYear(), today.getMonth() + 1, 0).getDate();
                setCurrentMonthDays(Array.from({ length: numberOfDays }, (_, index) => index + 1));
            } catch (error) {
    ...
    // Function to render attendance status for a specific day
    const renderAttendanceStatus = (userId, day) => {
        if (!attendanceData[userId]) {
            return <div className="p-4"> <span style={{ color: "grey" }}>N/A</span></div>;
        }

        // Find attendance for the specific day
        const attendanceForDay = attendanceData[userId].find(
            (attendance) => new Date(attendance.createdAt).getDate() === day
        );

        if (!attendanceForDay || attendanceForDay.status === "absent") {
            return <div className="p-4"> <img src={cross} alt="Absent" className="w-9 h-8" /></div>;
        } else if (attendanceForDay.status === "present") {
            return <div onClick={() => showModal(attendanceForDay)} className="p-4"><img src={check} alt="Present" className="w-9 h-8 cursor-pointer" /></div>;
        } else {
            return <div className="p-4"><span style={{ color: "grey" }}>N/A</span></div>;
        }
    };
    ...
    return (
        <div>
            <TitleCard title="Employee Attendance">
                        ...
                        <tbody>
                            {employees.map((employee) => (
                                <tr key={employee.userId}>
                                    ...
                                    {currentMonthDays.map((day) => (
                                        <td key={`${employee.userId}-${day}`}>
                                            {renderAttendanceStatus(employee.userId, day)}
                                        </td>
                                    ))}
                                </tr>
                            ))}
            ...
            {modal && <AttendenceModal closeModal={closeModal} attendance={selectedAttendance} />} {/* Pass selected attendance to modal */}
        </div>
    );
};
